                            ...
                            <div class="sidebar-account">
                                ...
                                <ul class="nav">
                                    ...
                                    @if (auth()->user()->subscription_status == 1)
                                        ...
                                        @if ($canViewProducts)
                                            <li class="nav-item">
                                                <a href="{{ route('choose-products') }}" class="nav-link">
                                                    <img src="{{ asset('front/assets/img/account/7.svg') }}" alt="img" />
                                                    {{ __('lang.choose_products') }}
                                                </a>
                                            </li>
                                        @else
                                            <li class="nav-item" style="color: #ccc;">
                                                <a href="#" class="nav-link disabled">
                                                    <img src="{{ asset('front/assets/img/account/7.svg') }}" alt="img" />
                                                    {{ __('lang.choose_products') }}
                                                    ({{ __('lang.Subscription_Required') }})
                                                </a>
                                            </li>
                                        @endif
                                    @endif
                                                    ...
                                                    <div class="form-clt">
                                                        ...
                                                        <div class="form">
                                                            <select name="race" id="race" class="form-control">
                                                                ...
                                                                <option value="Malay"
                                                                    @if (auth()->check() && auth()->user()->subscription_detail?->race === 'Malay') selected @endif>Malay
                                                                </option>
                                        ...
                                        <table class="table transaction-table" id="orderTable" style="width: 100%">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><a href="{{route('user.order.details', $order->id)}}">{{ $order->order_number }}</a></td>
                                                    <td>RM {{ $order->total_amount }}</td>
                                                    <td>{{ $order->status }}</td>
                                                    <td class="text-right">
                                                        <a href="{{route('user.order.invoice', $order->id)}}" class="action_btn" title="Generate Invoice"><i class="fas fa-file-invoice"></i></a>
                                                        @if ($order->status == 'failed')
                                                            @if($order->subscription_link)
                                                            <a href="{{$order->subscription_link}}" class="action_btn" title="Pay Now"> <i class="fa fa-credit-card"></i></a>
                                                            @else
                                                            <a href="#1" data-order="{{$order->razorpay_order_id}}" data-customer="{{$order->razorpay_customer_id ?? auth()->user()->customer_id}}" class="action_btn repay" title="Pay Now"> Pay Now</a>
                                                            @endif
                                                        @endif
                                                        @if($order->tracking_url)
                                                        <a href="{{$order->tracking_url}}" class="action_btn" title="Track Order"><i class="fa-solid fa-truck-fast"></i></a>
                                                        @endif
                                                    </td>
                                                </tr>
                                                @endforeach
                                        ...
                                        <div class="row align-items-center">
                                            <div class="col-md-6">
                                                <div class="points-display-section">
                                                    ...
                                                    <div>
                                                        <h3 class="points-number">{{ $totalPoints ?? 450 }}</h3>
    ...
    <script>
        document.getElementById('cancel-subscription').addEventListener('click', function() {
            Swal.fire({
                ...
                text: '{{ __('lang.cancel_subscription_message') }}',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
                confirmButtonText: '{{ __('lang.yes_cancel') }}',
                cancelButtonText: '{{ __('lang.no_keep') }}'
            }).then((result) => {
    ...
    @if (session('profile_incomplete'))
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    ...
                    text: "{{ session('profile_incomplete.message') }}",
                    icon: 'warning',
                    showCancelButton: false,
                    confirmButtonText: "{{ session('profile_incomplete.button_text') }}",
                    ...
                    if (result.isConfirmed) {
                        window.location.href = "{{ session('profile_incomplete.button_url') }}";
                    }
                });
            });
        </script>
        ...
        <style>
            ...
            .swal-confirm-button:hover {
                background-color: #d17e8f !important;
            }
        </style>
    @endif
    ...
    @if ($incompleteOrder == 1 || $abandoned_cart_type == 1)
        <script>
            Swal.fire({
                ...
                text: '{{ __('lang.order_incomplete') }}',
                icon: 'warning',
                confirmButtonText: '{{ __('lang.complete_order') }}'
            }).then((result) => {
                if (result.isConfirmed) {
                    window.location.href = "{{ $abandoned_cart_type == 1 ? route('checkout') : route('choose-products') }}";
                }
            });
        </script>
    @endif

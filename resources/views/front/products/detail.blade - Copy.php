                    ...
                    <div class="col-lg-6">
                        <div class="product-details-content">
                            ...
                            <div class="price-list">
                                ...
                                @if ($product->points > 0)
                                    <p>Buy this product now and earn <b>{{ $product->points }}</b> Points!</p>
                                @endif
                            ...
                            <div class="cart-wrp">
                                <div class="cart-quantity">
                                    <form id='myform' method='POST' class='quantity' action='#'>
                                        <input type='button' value='-' class='qtyminus minus' {{ $product->qty == 0 ? 'disabled' : '' }}>
                                        <input type='number' name='quantity' value='1' class='qty' {{ $product->qty == 0 ? 'disabled' : '' }}>
                                        <input type='button' value='+' class='qtyplus plus' {{ $product->qty == 0 ? 'disabled' : '' }}>
                                    </form>
                                </div>
                                ...
                                <p>Stock: <b>{{ $product->qty }}</b> available!</p>
                            </div>
                            <div class="shop-btn">
                                @if($product->qty > 0)
                                    <a href="javascript:void(0);" class="custom-rdxbtnr add-to-cart-btn"
                                        data-product-id="{{ $product->id }}" data-stock="{{ $product->qty }}" data-price="{{ $afterDiscount }}"
                                        id="addToCartBtn">
...
@section('scripts')
    <script>
        const productStock = {{ $product->qty }};
    </script>
    <script>
...
//             }
    ...
    if (quantity > stock) {
        toastr.error('Cannot add more than available stock.');
        return;
    }
    ...
    $.ajax({
        ...
        error: function(xhr) {
            let msg = xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to add to cart.';
            toastr.error(msg);
        }
    });
});

                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <div class="p-0">
                                    <h4 class="card-title mb-0 flex-grow-1">Monthly Product</h4>
                                </div>
                                <div class="p-0">
                                    <a href="{{ route('monthly-product.create') }}" class="btn btn-danger"
                                        data-name="productMonthlyAdd">
                                        <img src="{{ asset('admin/assets/images/svg/add.svg') }}" width="12"
                                            class="me-1"> Add Monthly Product
                                    </a>
                                </div>
                            </div>
                        </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            var table = $('#monthly-product-table').DataTable({
                processing: true,
                serverSide: true,
                responsive: true,
                autoWidth: false,
                ajax: {
                    url: "{{ route('monthly-product.get') }}",
                    type: "GET",
                ...
                fnInitComplete: function() {
                    $('#monthly-product-table').removeClass('d-none').fadeIn();
            ...
            $(document).on('click', '.dltBtn', function(e) {
                ...
                swal({
                    title: "Are you sure?",
                    text: "Once deleted, you will not be able to recover this monthly roducts!",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            type: 'POST',
                            url: "{{ route('monthly-product.destroy', ':id') }}".replace(
                                ':id', id),
                            data: {
                                _token: "{{ csrf_token() }}",
                                _method: 'DELETE'
                            ...
                            success: function(data) {
                                table.ajax.reload();
                                toastr.success('Product Bundle deleted successfully!');
                            },
                            error: function(xhr) {
                                toastr.error('Failed to delete Product Bundle!');
                            }
                        });
                    } else {
                        swal("Your product bundle is safe!");
                    }
                });
            });
        });

    ...
    @section('page-content')
        ...
        <div class="page-content">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <div class="p-0">
                                        @hasRoutePermission('admin.code.import')
                                            <button data-bs-toggle="modal" data-bs-target="#importModal" class="btn btn-light">
                                                <i class="ri ri-upload-2-line"></i> Import Codes</button>
            ...
            <!-- Import Modal -->
                ...
                <div class="modal-dialog modal-dialog-centered">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="importModalLabel">Import Gift Cards</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('admin.code.import') }}" id="import-code-form" enctype="multipart/form-data" method="post">
                            @csrf
                            @method('POST')
                            <div class="alert alert-info" role="alert">
                            <strong>Note:</strong> Please make sure the name 
                            of Product and Variant are exact same else system can not create Gift Card Code.
                            </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" form="import-code-form" class="btn btn-danger">Submit</button>
                    </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Error Handling Import Modal -->
                <div class="modal fade" id="ErrorimportModal" tabindex="-1" aria-labelledby="ErrorimportModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="ErrorimportModalLabel">Failed Rows for Gift Cards</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
    ...
    @section('scripts')
        ...
        @if (Session::has('import_failed'))
             <script>
                $(document).ready(function () {
                    $('#ErrorimportModal').modal('toggle');

                    let errorBody = $('#errorBody');
                    errorBody.html('');

                    let importFailed = @json(Session::get('import_failed'));

                    if (importFailed.codes_failed) {
                        importFailed.codes_failed.forEach(function(code) {
                            errorBody.append(`
                                <div class="alert alert-danger" role="alert">
                                    Gift Code "${code}" already exists
                                </div>
                            `);
                        });
                    }

                    if (importFailed.products_failed) {
                        importFailed.products_failed.forEach(function(product) {
                            errorBody.append(`
                                <div class="alert alert-danger" role="alert">
                                    Product Name "${product}" doesn't exist
                                </div>
                            `);
                        });
                    }

                    if (importFailed.variants_failed) {
                        importFailed.variants_failed.forEach(function(variant) {
                            errorBody.append(`
                                <div class="alert alert-danger" role="alert">
                                    Variant Name "${variant}" doesn't exist
                                </div>
                            `);
                        });
                    }

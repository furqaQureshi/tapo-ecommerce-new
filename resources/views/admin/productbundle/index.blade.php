                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <div class="p-0">
                                    <h4 class="card-title mb-0 flex-grow-1">Product Bundles</h4>
                                </div>
                                <div class="p-0">
                                    <a href="{{ route('product-bundle.create') }}" class="btn btn-danger"
                                        data-name="productBundleAdd">
                                        <img src="{{ asset('admin/assets/images/svg/add.svg') }}" width="12"
                                            class="me-1"> Add Product Bundle
                                    </a>
                                </div>
                            </div>
                        </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            var table = $('#product-bundles-table').DataTable({
                processing: true,
                serverSide: true,
                responsive: true,
                autoWidth: false,
                ajax: {
                    url: "{{ route('product-bundle.get') }}",
                    type: "GET",
                ...
                fnInitComplete: function() {
                    $('#product-bundles-table').removeClass('d-none').fadeIn();
            ...
            $(document).on('click', '.dltBtn', function(e) {
                ...
                swal({
                    title: "Are you sure?",
                    text: "Once deleted, you will not be able to recover this product bundle!",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            type: 'POST',
                            url: "{{ route('product-bundle.destroy', ':id') }}".replace(
                                ':id', id),
                            data: {
                                _token: "{{ csrf_token() }}",
                                _method: 'DELETE'

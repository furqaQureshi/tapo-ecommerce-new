    ...
    $documentOn.ready(function () {
        ...
        if ($('.brand-slider').length > 0) {
            const brandSlider = new Swiper(".brand-slider", {
                ...
                allowTouchMove: false, // user swipe disable karega
                speed: 2000, // jitna zyada speed utna slow smooth lagega
                autoplay: {
                    delay: 0, // marquee effect (continuous scroll)
                    disableOnInteraction: false,
                },
                ...
                navigation: false, // arrows hide
                pagination: false, // dots hide
            });
        }
...
$(document).ready(function () {
    ...
    function updateCheckoutStep() {
        ...
        giftPrice = selectedProducts.length === 2 ? 30 : 0;

        selectedProducts.forEach(product => {
            let priceText = product.addonPrice > 0 ? `+ RM${product.addonPrice}` : 'Free';
            $selectedProductsContainer.append(`
                <div class="d-flex flex-column align-items-center justify-content-center text-center p-2 position-relative" data-product-id="${product.id}" style="position: relative;">
            ...
            setTimeout(function() {
                const newItem = $(`
                    <div class="d-flex flex-column align-items-center justify-content-center text-center p-2">
                        ...
                        <span class="mt-1">Free</span>
                    </div>
                `).hide();
                $selectedProductsContainer.append(newItem);
                newItem.fadeIn(500, function() {
                    toastr.success('🥳 We have added another mystery gift to make it RM100 & above 🎁', 'CONGRATULATIONS');
                });
            }, 500);
        }
        ...
        //HeartDisk
        const planPrice = (selectedProducts.length === 3 ? (selectedPlan ? parseFloat(selectedPlan.price) : 0) : 0);
         total = productTotal + planPrice + giftPrice;
        const totalAddon = selectedProducts.reduce((sum, p) => sum + (p.addonPrice || 0), 0);
        const proLength = $('.close-product').length;
        if(proLength==2)
        {
            if(total < 100){             
                total = 100;      
    ...
    function showVariantModal($button) {
        let html = '';
        const attributesData = $button.data('attributes');

        // Log attributes for debugging
        console.log('Attributes Data:', attributesData);

        // Check if attributes exist and are valid
        if (attributesData && attributesData !== '0' && attributesData !== 0) {
            let attributes = [];
            try {
                attributes = typeof attributesData === 'string' ? JSON.parse(attributesData) : attributesData;
            ...
            if (Array.isArray(attributes) && attributes.length > 0) {
                ...
                attributes.forEach(attr => {
                    ...
                    groups[attr.attribute_id].push(attr);
                });

                for (const attrId in groups) {
                    html += `
                        <div class="form-group mb-3">
                            <label for="variant-select-${attrId}" class="form-label">${groups[attrId][0].attribute_name}</label>
                            <select id="variant-select-${attrId}" class="form-select variant-select" data-attr-name="${groups[attrId][0].attribute_name}" data-attr-id="${attrId}">
                                <option value="">Choose...</option>
                    `;
                    groups[attrId].forEach(option => {
                        html += `<option value="${option.id}" data-price="${option.price}">${option.attribute_value}</option>`;
                    });
                    html += `</select></div>`;
                }
            }
        ...
        // Use dummy variations if no valid attributes
        if (!html) {
            html = `
                <div class="form-group mb-3">
        ...
        $('#variantModal .modal-body').html(html);
        $('#addNowBtn').prop('disabled', true);
        $('#variantModal').data('button', $button);
        $('#variantModal').modal('show');
    }

    $(document).on('change', '.variant-select', function () {
        const allSelected = $('.variant-select').toArray().every(select => $(select).val() !== '');
        $('#addNowBtn').prop('disabled', !allSelected);
    });

    $(document).on('click', '#addNowBtn', function () {
        const $modal = $('#variantModal');
        const $button = $modal.data('button');
        const productId = $button.data('product-id');
        const basePrice = parseFloat($button.data('price'));
        const baseAddonPrice = parseFloat($button.data('addon-price'));
        const $productItem = $button.closest('.product-collection-item');
        const productName = $productItem.find('h4 a').text();
        const productImage = $productItem.find('img').attr('src');
        ...
        $('.variant-select').each(function () {
            ...
            if (valId) {
                ...
                const attrPrice = parseFloat(option.data('price')) || 0;
                extraPrice += attrPrice;
                variants.push({
                    id: $(this).data('attr-id'),
                    name: $(this).data('attr-name'),
                    // attribute_value_id: valId,
                    value: option.text().replace(/ \+\w+/, '')
                });
            }
        });

        // const addonPrice = baseAddonPrice + extraPrice;
        const addonPrice = baseAddonPrice;

        if (selectedProducts.length >= 2) {
            atoastr.errorlert('You can only select up to 2 products for the bundle.');
            return;
        }
        ...
        $button.data('selected', true).addClass('selected');
        $button.html('<span class="icon">♡</span> Added');
        const $checkbox = $(`#${productId}-checkbox`);
        ...
        if (!$(e.target).closest('.shopc-button').length && currentStep === 1) {
            ...
            if ($button.length) {
                ...
                if (isSelected) {
                    ...
                    if (attributes === 0 || attributes === '0' || !attributes) {
                        toggleButton($button);
                    } else {
                        showVariantModal($button);
                    }
                }
            }
        }
    ...
    $(document).on('click', '.close-product', function (e) {
        ...
        const $button = $(`.shopc-button[data-product-id="${productId}"]`);
        if ($button.length) {
            toggleButton($button);
        }
    });
    ...
    function setSelectedPlan($planInput) {
        ...
        var formattedDate = renewalDate.toISOString().split('T')[0];
...
$(document).ready(function() {
    $('.toggle-arrow').on('click', function() {
        const $subcategoryList = $(this).parent().next('.subcategories');
